...
// import type { Token, Network } from '@/lib/crossChainSwap/types';
// import { tokens, networks } from '@/lib/crossChainSwap/data';
...
//       if (!excludeToken) {
//         // No token selected yet — show all tokens
//         return true;
//       }

//       const isExcludeIDRX = excludeToken.symbol === IDRX_SYMBOL;
//       const isCurrentIDRX = firstToken.symbol === IDRX_SYMBOL;

//       // Only allow IDRX ↔ non-IDRX pairs
//       return isExcludeIDRX ? !isCurrentIDRX : isCurrentIDRX;
//     })
//     .filter((symbol) => {
//       const tokens = tokensBySymbol[symbol];
...
//       if (tokens.length > 1) {
//         return tokens.some((token) => token.address !== excludeToken.address);
//       }
...
export default function TokenSelector({
  ...
  // Filter out networks that match the excludeToken's network
  const getFilteredTokensForSymbol = (symbol: string) => {
    if (!excludeToken) return tokensBySymbol[symbol];

    return tokensBySymbol[symbol].filter(
      (token) => token.network !== excludeToken.network
    );
  };

  const handleTokenClick = (symbol: string) => {
    const filteredTokensForSymbol = getFilteredTokensForSymbol(symbol);

    // If there's only one valid network for this token after filtering, select it directly
    if (filteredTokensForSymbol.length === 1) {
      onSelect(filteredTokensForSymbol[0]);
      resetAndClose();
      return;
    }
    ...
    >
      <DialogContent className="bg-gray-900 text-white border-gray-800 max-w-md">
        ...
        <div className="max-h-[400px] overflow-y-auto pr-2 -mr-2">
          {filteredTokenSymbols.map((symbol) => {
            const filteredTokensForSymbol = getFilteredTokensForSymbol(symbol);
            // Skip rendering if no valid tokens left after filtering
            if (filteredTokensForSymbol.length === 0) return null;

            const isExpanded = expandedToken === symbol;
            const networkCount = filteredTokensForSymbol.length;
            ...
            return (
              <div key={symbol} className="mb-1">
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full overflow-hidden bg-gray-700 flex items-center justify-center">
                      <Image
                        src={
                          filteredTokensForSymbol[0].logoURI ||
                          `/placeholder.svg?height=32&width=32&text=${symbol}`
                        }
                        alt={symbol}
                        className="w-full h-full object-cover"
                    ...
                    <div className="text-left">
                      <div className="font-medium">{symbol}</div>
                      <div className="text-sm text-gray-400">
                        {filteredTokensForSymbol[0].name}
                        {networkCount > 1 && ` · ${networkCount} networks`}
                      </div>
                    </div>
                  </div>
                ...
                {isExpanded && (
                  <div className="bg-gray-800 rounded-b-lg overflow-hidden transition-all duration-300 ease-in-out">
                    {filteredTokensForSymbol.map((token) => {
                      if (
                        excludeToken &&
                        token.address === excludeToken.address
                      )
